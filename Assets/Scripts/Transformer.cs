...
public class Transformer : MonoBehaviour
{
	...
	public IEnumerator TransformToCarCorutine()
	{
		...
		if (isPlayer)
		{
			...
			CameraEffect cameraEffect = EffectManager.Instance.Create(Type.SprintShake);
			cameraEffect.Stop();
		}
		...
		car.GetComponent<Rigidbody>().linearVelocity = rb.linearVelocity;
	...
	public IEnumerator TransformToRobotCorutine(GameObject existingCar = null, HitEntity target = null)
	{
		...
		if (isPlayer)
		{
			...
			PlayerInteractionsManager.Instance.InstantExitVehicle(isTransformer: true);
		}
		else
		{
		...
		else
		{
			...
			if (target != null)
			{
				baseNpc.ChangeController(BaseNPC.NPCControllerType.Smart, out BaseControllerNPC _);
