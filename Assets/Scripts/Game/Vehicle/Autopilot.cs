...
namespace Game.Vehicle
{
	public class Autopilot : MonoBehaviour
	{
		...
		public UnityEvent m_OnInit;

		public UnityEvent m_DeInit;
		...
		public DrivableVehicle Vehicle => drivableVehicle;

		public Rigidbody Root => rootBody;

		public void OnSensorStay(Collider otherCollider, CollisionTrigger.Sensor sensor)
		{
			if (otherCollider.attachedRigidbody != null)
			{
				Vector3 rhs = otherCollider.attachedRigidbody.linearVelocity - rootBody.linearVelocity;
		...
		public void InitChase(Rigidbody vehicleBody)
		{
			...
			vehicleBody.linearVelocity = vehicleBody.transform.forward * (5f / 18f) * Mathf.Min(cruiseVel, 100f);
			m_OnInit.Invoke();
		}
		...
		public virtual void DropPassangers()
		{
			...
			if (componentInChildren != null)
			{
				...
				componentInChildren.InitOutOfVehicle(force: false, player, isReplaceOnRagdoll: false);
				drivableVehicle.OpenVehicleDoor(VehicleDoor.LeftDoor, isGettingIn: false);
		...
		private void FixedUpdate()
		{
			...
			Vector3 vector = rootBody.transform.InverseTransformDirection(rootBody.linearVelocity);
			speed = vector.z;
			if (rootBody.linearVelocity.magnitude * 3.6f < 3f && !isStacked)
			{
		...
		private void ChaseFixedUpdate()
		{
			...
			if (num < MaxDistanceToDrop && target.linearVelocity.magnitude < 1.388889f)
			{
				...
				if (speed < 5f && exitBlockTime <= 0f)
				{
					exitBlockTime = 2f;
					NavMeshHit hit;
					if (!drivableVehicle.IsDoorBlockedOffset(DropPassengersBlockingLayer, rootBody.transform, out Vector3 _) && !drivableVehicle.DeepInWater && !NavMeshAgent.SamplePathPosition(-1, 50f, out hit) && (hit.mask & waterMask) == 0 && !(drivableVehicle is DrivableTank))
					{
						isWorking = false;
						DropPassangers();
					}
		...
		private void WaypointTourFixedUpdate()
		{
			if (waypoints != null && waypoints.Length >= 1)
			{
				...
				if (Vector3.Distance(rootBody.transform.position, transform.position) < 10f)
				{
				...
				ProceedSensors(out float resultThrottle, ref resultSteer, cruiseVel);
				ProceedStack(ref resultThrottle, ref resultSteer);
				ThrottleFixedUpdate(resultThrottle);
				SteeringFixedUpdate(resultSteer);
			}
		...
		private void SteeringFixedUpdate(float steer)
		{
			...
			if (rootBody != null)
			{
				if (drivableVehicle != null && drivableVehicle is DrivableTank)
				{
					rootBody.AddRelativeTorque(d * Vector3.up * steer * 10000f * 2000f * Time.deltaTime);
				}
				else
				{
