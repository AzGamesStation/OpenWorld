...
namespace Game.Vehicle
{
	public class TankController : VehicleController
	{
		...
		private void Inputs()
		{
			...
			speed = MainRigidbody.linearVelocity.magnitude * 3f;
			acceleration = 0f;
			Vector3 vector = MainRigidbody.transform.InverseTransformDirection(MainRigidbody.linearVelocity);
			float z = vector.z;
			acceleration = (z - lastVelocity) / Time.deltaTime;
			lastVelocity = z;
			MainRigidbody.linearDamping = Mathf.Clamp(acceleration / 10f, 0f, 1f);
		...
		private void SmokeRate()
		{
			...
			if ((bool)initedNormalExhaustGas)
			{
				initedNormalExhaustGas.emit = (speed < 15f);
			}
			if ((bool)initedHeavyExhaustGas)
			{
				initedHeavyExhaustGas.emit = (Mathf.Abs(motorInput) > 0.1f || Mathf.Abs(steerInput) > 0.1f);
			}*/
		}
