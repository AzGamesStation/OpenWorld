...
namespace Game.Vehicle
{
	public class CarController : VehicleController
	{
		...
		public PhysicsMaterial minFrictionMaterial;

		public PhysicsMaterial standartFrictionMaterial;
		...
		public override void Init(DrivableVehicle drivableVehicle)
		{
			...
			if (carSpecific != null && vehicleSound)
			{
		...
		private void Update()
		{
			...
			bool left = Controls.GetButton("TurnLeft");
			bool right = Controls.GetButton("TurnRight");
			if (steerWheels != null)
			{
				...
				foreach (WheelCollider wheelCollider in array)
				{
					...
					if (left)
					{
						Steer = Mathf.Lerp(Steer, -1, Time.deltaTime);
						wheelCollider.steerAngle = Steer * 30f;
						//if (!once)
						//{
						//	once = true;
						//	if (Taturial.instance)
						//	{
						//		Taturial.instance.EndStep();
						//	}
						//}
		...
		private void ProceedEngineSound(int id)
		{
			if (vehicleSound)
			{
				...
				{
					PointSoundManager.Instance.PlaySoundAtPoint(base.transform.position, vehicleSound.GearShiftSound);
