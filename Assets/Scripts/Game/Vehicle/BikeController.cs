	...
	public class BikeController : VehicleController
	{
		...
		public IEnumerator DropFromDelay()
		{
			...
			if (IsInitialized)
			{
				...
				PlayerInteractionsManager.Instance.SwitchSkeletons(on: false, isAnimatorRebind: false);
				player.ResetRotation();
				player.ReplaceOnRagdoll(canWakeUp: true, out GameObject initRagdoll);
			
				MainRigidbody.constraints = RigidbodyConstraints.None;
				Transform transform2 = initRagdoll.transform;
				Vector3 eulerAngles = initRagdoll.transform.eulerAngles;
				float x = eulerAngles.x;
				Vector3 eulerAngles2 = initRagdoll.transform.eulerAngles;
				transform2.eulerAngles = new Vector3(x, eulerAngles2.y, 0f);
		...
		private void Jump()
		{
			if (RearWheelCollider.GetGroundHit(out WheelHit _))
			{
