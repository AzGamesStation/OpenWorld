...
namespace Game.Vehicle
{
	...
	public class DrivableVehicle : MonoBehaviour, IInitable
	{
		...
		public Rigidbody MainRigidbody => mainRigidbody ?? (mainRigidbody = GetComponent<Rigidbody>());
		...
		{
			return MainRigidbody.linearVelocity.magnitude < 20f && !isTransformer;
		}
		...
		public void ChangeBodyColor(Renderer[] renderers)
		{
			...
			foreach (Renderer renderer in renderers)
			{
				...
				for (int j = 0; j < num; j++)
				{
					if (sharedMaterials[j].name.ToLower().Contains("#colored"))
					{
						num2 = j;
					}
				}
				if (num2 > -1)
				{
					sharedMaterials[num2] = bodyMaterial;
				}
		...
		public virtual bool AddObstacle()
		{
			...
			obstacle = base.transform.gameObject.AddComponent<NavMeshObstacle>();
		...
		public virtual void OnCollisionEnter(Collision c)
		{
			...
			if (c.collider.gameObject.layer == bigDynamicLayerNumber)
			{
				...
				{
					vehicleStatus.OnHit(DamageType.Collision, CurrentDriver, num * DamagePerSpeed, c.contacts[0].point, (c.contacts[0].point - base.transform.position).normalized);
				}
			}
			...
			{
				vehStatus.OnHit(DamageType.Collision, CurrentDriver, num * DamagePerSpeed, c.contacts[0].point, c.contacts[0].normal.normalized);
			}
		...
		public virtual bool IsDoorBlockedOffset(LayerMask blockedLayerMask, Transform driver, out Vector3 offset, bool horizontalCheckOnly = true)
		{
			...
			UnityEngine.Debug.DrawRay(vector2, -capsuleCollider.transform.up * capsuleCollider.height * 0.8f, Color.yellow, 5f);
			RaycastHit hitInfo;
			bool flag = !Physics.Raycast(vector, -base.transform.right, out hitInfo, ExitLeftMinDistance, blockedLayerMask);
			bool flag2 = horizontalCheckOnly || !Physics.Raycast(vector2, -capsuleCollider.transform.up, out hitInfo, capsuleCollider.height * 0.8f, blockedLayerMask);
			if (flag && flag2)
			{
		...
		protected virtual void FixedUpdate()
		{
			...
			{
				CurrentDriver.Drowning(waterDepth);
		...
		protected void DrowningDummyDriver()
		{
			...
			{
				DummyDriver.DropRagdoll(null, Vector3.zero, canWakeUp: false, isOnWater: true, waterDepth);
				ResetDriver();
			}
		}
		...
		public void Effects(Collision c)
		{
			...
			{
				if (c.gameObject.layer != smallDynamicLayerNumber || !(num > 2f))
				{
					return;
				}
