...
namespace Game.Character
{
    public class PlayerInteractionsManager : MonoBehaviour
    {
        ...
        public Button GetInPropsButton;
        ...
        public float tweakTimer;

        public float tweakTimeOut;
        ...
        public bool IsPlayerInMetro
        {
            get
            {
                if (MetroManager.InstanceExists)
                {
                    return MetroManager.Instance.InMetro;
                }
                return false;
            }
        }
        ...
        {
            if (animationController.tweakStart && LastDrivableVehicle is DrivableCar)
            {
                animationController.MainAnimator.enabled = false;
                //skeletonOfCharacter.transform.position = LastDrivableVehicle.VehiclePoints.DriverPosition.position;
                ...
                //MoveCharacterToSitPosition(characterHips, LastDrivableVehicle.VehiclePoints.DriverPosition, TweakingTic);
                //TweakingSkeleton(characterHips, LastDrivableVehicle.VehiclePoints.DriverPosition, TweakingTic);
            }
            else if (LastDrivableVehicle)
            {
        ...
        public void GetIntoVehicle()
        {
            ...
            if (CharacterPropsController.instance)
            {
            ...
            switch (LastDrivableVehicle.GetVehicleType())
            {
                ...
                case VehicleType.Tank:
                    ...
                    {
                        GlobalContants.tryVehicle = true;
                        RewardedAdsController.Instance.ShowRewarded("FreeTank");
                    }
                    else
                    {
                ...
                case VehicleType.Motorbike:
                    ...
                    {
                        GlobalContants.tryVehicle = true;
                        RewardedAdsController.Instance.ShowRewarded("FreeMotorBike");
                    }
                    else
                    {
                ...
                case VehicleType.Copter:
                    if (!LastDrivableVehicle.WaterSensor.InWater)
                    {
                        ...
                        {
                            GlobalContants.tryVehicle = true;
                            RewardedAdsController.Instance.ShowRewarded("FreeCopter");
                        }
                        else
                        {
        ...
        public void InteractionWithVehicle()
        {
            if (InteractionsAllowed)
            {
                ...
                characterHips = skeletonOfCharacter;
                if (LastDrivableVehicle is DrivableBike)
                {
                    skeletonInVehicle = (LastDrivableVehicle as DrivableBike).metarig;
                }
                else
                {
                    skeletonInVehicle = LastDrivableVehicle.controller.VehicleSpecific.PlayerSkeleton.Find("metarig");
                }
                driverHips = skeletonInVehicle;
                Player.transform.parent = LastDrivableVehicle.transform;
                Player.GetInOutVehicle(isInOut: true, forceEnter, LastDrivableVehicle);
                ...
                Player.ProceedDriverStatus(LastDrivableVehicle, isGettingIn: true, delayedActivateTime);
                ClearNearestVehicle();
        ...
        public bool IsPossibleGetOut()
        {
            ...
            if (animationController.tweakStart && LastDrivableVehicle.GetVehicleType() != VehicleType.Motorbike)
            {
                return false;
            }
            if (!inVehicle)
        ...
        public void GetOutFromVehicle(bool isRaggdol = false)
        {
            ...
            if (LastDrivableVehicle.DeepInWater)
            {
                ...
                Player.GetInOutVehicle(isInOut: false, force: false, LastDrivableVehicle, crash: true);
            ...
            else
            {
                ...
                {
                    case VehicleType.Car:
                        GetInOutCar(isIn: false);
                        break;
                    case VehicleType.Bicycle:
                        GetInOutBike(isIn: false);
                        break;
                    case VehicleType.Tank:
                        GetInOutTank(isIn: false);
                        break;
                    case VehicleType.Motorbike:
                        GetInOutMoto(isIn: false, isRaggdol);
                        break;
                    case VehicleType.Copter:
                        GetInOutHelicopter(isIn: false);
                        break;
                    case VehicleType.Mech:
                        GetInOutMech(isIn: false);
                        break;
            ...
            Player.ProceedDriverStatus(LastDrivableVehicle, isGettingIn: false);
        ...
        private void GetInOutCar(bool isIn)
        {
            ...
            float magnitude = LastDrivableVehicle.MainRigidbody.linearVelocity.magnitude;
            if (magnitude >= 100f && LastDrivableVehicle.HasExitAnimation())
            {
                characterHips = skeletonOfCharacter;
                Player.JumpOutFromVehicle(canWakeUp: true, 1.05f, isReplaceOnRagdoll: true, lookOnVehicleFirst: true, jumpInAir: false, revertYRotateOnFinish: false, LastDrivableVehicle);
                return;
            }
            if (LastDrivableVehicle.controller != null)
            {
            ...
            characterHips = skeletonOfCharacter;
            Player.GetInOutVehicle(isInOut: false, force: false, LastDrivableVehicle, crash);
            Transform transform = Player.transform;
            Vector3 eulerAngles = Player.transform.eulerAngles;
            transform.eulerAngles = new Vector3(0f, eulerAngles.y, 0f);
        }
        ...
        private void GetInOutBike(bool isIn)
        {
            ...
            else
            {
                ...
                Player.GetInOutVehicle(isInOut: false, force: false, LastDrivableVehicle);
            }
        }
        ...
        private void GetInOutMoto(bool isIn, bool isRagdoll = false)
        {
            if (isIn)
            {
                ...
                LastDrivableVehicle.MainRigidbody.constraints = RigidbodyConstraints.FreezeRotationZ;
                LastDrivableVehicle.MainRigidbody.linearVelocity = Vector3.zero;
                Player.MoveToCar = true;
            ...
            else
            {
                ...
                //    Player.JumpOutFromVehicle(canWakeUp: true, 0.2f, magnitude >= 20f, lookOnVehicleFirst: true, jumpInAir: false, revertYRotateOnFinish: false, LastDrivableVehicle);
                ...
                //{
                    ...
                    Player.GetInOutVehicle(isInOut: false, force: false, LastDrivableVehicle, isRagdoll);
                //}
            }
        }
        ...
        private void GetInOutHelicopter(bool isIn)
        {
            ...
            else
            {
                ...
                {
                    Player.JumpOutFromVehicle(canWakeUp: true, 2.7f, isReplaceOnRagdoll: false, lookOnVehicleFirst: false, jumpInAir: true, revertYRotateOnFinish: true, LastDrivableVehicle);
                }
            }
        }
        ...
        private void GetInOutTank(bool isIn)
        {
            ...
            else
            {
                characterHips = skeletonOfCharacter;
                Player.rootModel.SetActive(value: true);
                Player.GetInOutVehicle(isIn, force: false, LastDrivableVehicle);
            }
        }
                ...
                case VehicleType.Car:
                    {
                        ...
                        Player.JumpOutFromVehicle(canWakeUp: false, animationTimeLength, isReplaceOnRagdoll: true, lookOnVehicleFirst: false, jumpInAir: false, revertYRotateOnFinish: false, LastDrivableVehicle);
                        break;
                    }
                case VehicleType.Copter:
                    Player.transform.parent = null;
        ...
        public void ResetCharacterHipsPlace()
        {
            characterHips = skeletonOfCharacter;
        }
        ...
        public void SwitchInOutVehicleButtons(bool vehicleIn, bool vehicleOut, bool wait = false)
        {
            ...
            if (!wait && !outFromCar && !CharacterPropsController.instance.isSkating )
            {
                if(vehicleIn && LastDrivableVehicle.gameObject.tag=="TryVehicle")
                {
                        GetInVehicleButton.transform.GetChild(0).gameObject.SetActive(true);
                    GetInVehicleButton.gameObject.SetActive(vehicleIn);
                    GetOutVehicleButton.gameObject.SetActive(vehicleOut);
                }
                else
                {
                    GetInVehicleButton.transform.GetChild(0).gameObject.SetActive(false);
                    GetInVehicleButton.gameObject.SetActive(vehicleIn);
                    GetOutVehicleButton.gameObject.SetActive(vehicleOut);
                }
        ...
        private void ChooseNearestVehicle()
        {
            ...
            else if (!Player.MoveToCar)
            {
                ...
                nearestVehicles.RemoveAll(list.Contains);
            }
        }
        ...
        private IEnumerator InVehicleButton()
        {
            ...
            if (!CharacterPropsController.instance.isSkating)
            {
        ...
        public DrivableVehicle InstantExitVehicle(bool isTransformer = false)
        {
            ...
            Player.transform.parent = null;
            SwitchSkeletons(on: false);
            Player.GetInOutVehicle(isInOut: false, force: false, drivableVehicle);
            Player.ResetRotation();
            ...
            Player.ProceedDriverStatus(drivableVehicle, isGettingIn: false);
            outFromCar = true;
            inVehicle = false;
            GameEventManager.Instance.RefreshQwestArrow();
            sitInVehicle = false;
        ...
        public void InstantEnterVehicle(DrivableVehicle vehicle, bool isTransformer = true)
        {
            ...
            if (!isTransformer)
            {
                ...
                characterHips = skeletonInVehicle;
            }
            if (!isTransformer)
            {
                MovePlayerToVehicle = true;
            }
            Player.GetInOutVehicle(isInOut: true, forceEnter, vehicle);
            vehicle.Drive(Player);
            Player.ProceedDriverStatus(vehicle, isGettingIn: true);
            ClearNearestVehicle();
            SwitchInOutVehicleButtons(vehicleIn: false, vehicleOut: true, wait: true);
