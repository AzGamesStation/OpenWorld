...
namespace Game.Character.Modes
{
    ...
    public class ThirdPersonCameraMode : CameraMode
    {
        ...
        // edited by abdul rafay
        private void UpdateAlwaysOnBack(ref Vector3 dir)
        {
            if ((!hasRotated || (hasRotated && (((Vector2)InputManager.GetInput(InputType.Move).Value).sqrMagnitude > Mathf.Epsilon))) && !(((Vector2)InputManager.GetInput(InputType.Rotate).Value).sqrMagnitude > Mathf.Epsilon))
            {
                float @float = config.GetFloat("DefaultYRotation");
                ...
                Game.Character.Utils.Math.ToCartesian(num2, num, out Vector3 dir2);
                Quaternion b = Quaternion.FromToRotation(dir, dir2);
                Quaternion rotation = Quaternion.Slerp(Quaternion.identity, b, Time.deltaTime * 0.5f * 10f);
                dir = rotation * dir;
            }
        }
        ...
        private void UpdateAutoReset(ref Vector3 dir)
        {
            ...
            if ((resetTimeout < 0f && hasRotated) ||
                (!hasRotated && !(((Vector2)InputManager.GetInput(InputType.Rotate).Value).sqrMagnitude > Mathf.Epsilon)) 
                //|| (!hasRotated && !(((Vector2)InputManager.GetInput(InputType.Move).Value).y < 0))
                )
            {
                float @float = config.GetFloat("DefaultYRotation");
                float num = (0f - @float) * ((float)System.Math.PI / 180f);
