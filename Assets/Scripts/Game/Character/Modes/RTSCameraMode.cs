	...
	public class RTSCameraMode : CameraMode
	{
		...
		private void DragCamera(Vector2 mousePosition)
		{
			...
			else
			{
				...
				if (GameInput.FindWaypointPosition(mousePosition, out Vector3 pos))
				{
		...
		private void UpdateDistance()
		{
			...
			{
				ref Vector3 cameraTarget = ref base.cameraTarget;
		...
		private void UpdateCollision()
		{
			if ((bool)collision)
			{
				collision.ProcessCollision(cameraTarget, cameraTarget, UnityCamera.transform.forward, targetDistance, out float _, out float _);
			}
		}
