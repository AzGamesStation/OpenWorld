...
namespace Game.Character.CharacterController
{
	public class Human : HitEntity, IInitable
	{
		...
		public override void Drowning(float waterHight, float drowningDamageMult = 1f)
		{
			if (!base.IsDead)
			{
				OnHit(DamageType.Water, null, DamagePerDrow * drowningDamageMult * Time.deltaTime, Vector3.zero, Vector3.zero);
			}
		}
		...
		{
			if (!animController.SurfaceSensor.AboveGround && !animController.SurfaceSensor.InWater || isPropsOn)
			{
				return;
			}
			//if(!isPropsOn)
   ...
   //         }
			...
			if (!AudioSource.isPlaying && footsteps.Length >0)
			{
			...
			if (timeToClipEnd <= 0f && footsteps.Length > 0)
			{
				footstepIndex++;
				SplashIndex++;
		...
		protected virtual void OnDieSpecific()
		{
			if (!currentRagdoll)
			{
				ReplaceOnRagdoll(canWakeUp: false, IsInWater);
			}
		...
		public virtual void ReplaceOnRagdoll(bool canWakeUp, bool isDrowning = false)
		{
			...
			{
				ReplaceOnRagdoll(canWakeUp, out GameObject _, isDrowning);
			}
		}
		...
		public virtual void ReplaceOnRagdoll(bool canWakeUp, Vector3 forceVector, bool isDrowning = false)
		{
			...
			{
				ReplaceOnRagdoll(canWakeUp, out GameObject initRagdoll, isDrowning);
				Transform transform = initRagdoll.transform.Find("mixamorig:Hips");
				if (transform == null)
				{
					transform = initRagdoll.transform.Find("metarig").Find("mixamorig:Hips");
				}
				Rigidbody component = transform.GetComponent<Rigidbody>();
				component.AddForce(forceVector, ForceMode.VelocityChange);
			}
		}
		...
		public virtual void ReplaceOnRagdoll(bool canWakeUp, out GameObject initRagdoll, bool isDrowning = false)
		{
			...
			currentRagdoll.transform.position = new Vector3(base.transform.position.x, base.transform.position.y + 0.5f, base.transform.position.z);
			...
			if (isDrowning)
			{
				...
				ragdollDrowning.Init(currentRagdoll.transform.Find("metarig").Find ("mixamorig:Hips"), animController.SurfaceSensor.CurrWaterSurfaceHeight);
			}
		...
		public void ClearCurrentRagdoll()
		{
			if (!(currentRagdoll == null))
			{
				...
				Rigidbody[] rgs = currentRagdoll.GetComponentsInChildren<Rigidbody>();
				foreach (var rg in rgs)
				{
					rg.isKinematic = true;
				}
				//currentRagdoll.transform.FindChild("00").transform.position = new UnityEngine.Vector3 (0, 0, 0);
		...
		protected void CopyTransformRecurse(Transform mainModelTransform, GameObject ragdoll)
		{
			...
			Rigidbody[] rgs = ragdoll.GetComponentsInChildren<Rigidbody>();
			foreach (var rg in rgs)
			{
		...
		protected virtual void OnCollisionEnter(Collision col)
		{
			if ((bool)RagdollWakeUper && !IsInWater)
			{
				...
				if (!(Mathf.Abs(num) < OnCollisionKnockingTreshold))
				{
					...
					OnHit(DamageType.Collision, owner, num * 1f, col.contacts[0].point, col.contacts[0].normal.normalized);
		...
		{
			//return model.transform.Find("metarig/habshi@T-Pose/mixamorig:Hips");
			return model.transform.Find("metarig/mixamorig:Hips");
		}
	}
}
