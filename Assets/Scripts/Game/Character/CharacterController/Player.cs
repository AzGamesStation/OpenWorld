...
namespace Game.Character.CharacterController
{
    public class Player : Human
    {
        ...
        public WeaponController WeaponController => weaponController ?? (weaponController = GetComponent<WeaponController>());
        ...
        public void UpdateStats(bool resetCurrent = true)
        {
            ...
            {
                Health.Setup(Health.Current, playerStat);
            }
            Health.RegenPerSecond = stats.GetPlayerStat(StatsList.HealthRegeneration);
            ...
            stats.UpdateStats(resetCurrent);
        ...
        private void OnFallImpact(Vector3 velocityVector)
        {
            ...
            if (num > 10f && !IsInWater)
            {
                ...
                OnHit(DamageType.Collision, null, damage, base.transform.position, base.transform.up);
                if (!base.RDCollInvul)
                {
        ...
        private void MoveToCarFunction(Vector2 moveInput)
        {
            if (MoveToCar)
            {
                ...
                else if (agent != null)
                {
                    ...
                    if (sqrMagnitude <= 0.5f && PlayerInteractionsManager.Instance.IsAbleToInteractWithVehicle() && !PlayerInteractionsManager.Instance.LastDrivableVehicle.IsDoorBlockedOffset(BlockedLayerMask, base.transform, out Vector3 _))
                    {
                        agent.enabled = false;
                        MoveToCar = false;
                        PlayerInteractionsManager.Instance.InteractionWithVehicle();
        ...
        public void ProceedStamina()
        {
            ...
            if (stats.stamina.Current < StaminaPerJump && HelpfullAdsManager.Instance != null)
            {
            ...
            if (stats.stamina.Current < stats.stamina.Max * 0.15f)
            {
                ...
                {
                    UI.UIGame.Instance.staminaPanel.SetActive(true);
            ...
            {
                WeaponController.UpdateAmmoText(WeaponController.CurrentWeapon as EnergyAmmoRangedWeapon);
            }
        }
        ...
        protected override void FixedUpdate()
        {
            ...
            if (ToHighToFly)
            {
                OnHit(DamageType.Instant, null, animController.OverHighDamagePerTic * Time.deltaTime, Vector3.zero, Vector3.zero);
            ...
            inputs.laser = inputManager.GetInput(InputType.Laser, defaultValue: false);
            inputs.flame = inputManager.GetInput(InputType.Flame, defaultValue: false);
            inputs.webthrow = inputManager.GetInput(InputType.Webthrow, defaultValue: false);
            inputs.ballonthrow = inputManager.GetInput(InputType.Ballonthrow, defaultValue: false);
            inputs.bubblethrow = inputManager.GetInput(InputType.Bubblethrow, defaultValue: false);
            ProceedStamina();
            ...
            if (GetMetarig())
                GetMetarig().rotation = new Quaternion(0, 0, 0, 0);
            ...
            else if (animController.UseSuperFly && inputs.fly && animController.CanStartSuperFly)
            {
                if (Time.time - moveStateSwitchTime > 0.3f)
                {
                    ...
                    {
                        Debug.Log("wings are off now");
                        GlobalContants.isWingsOn = false;
                        ResetMoveState();
                        needToSwitchDefMoveState = true;
                    }
            ...
            animController.Move(new AnimationController.Input
            {
                ...
                laser = inputs.laser,
                flame = inputs.flame,
                webthrow = inputs.webthrow,
                ballonthrow = inputs.ballonthrow,
                bubblethrow = inputs.bubblethrow,
                fly = IsFlying
            });
        ...
        public bool CheckDrowning()
        {
            ...
            if (flag)
            {
                OnHit(DamageType.Water, null, DamagePerDrow * Time.deltaTime, Vector3.zero, Vector3.zero);
            }
            return flag;
        }
        ...
        private IEnumerator JumpOut(bool canWakeUp, float animationTimeLength, bool isReplaceOnRagdoll, bool lookOnVehicleFirst, bool jumpInAir, bool revertYRotateOnFinish)
        {
            ...
            animController.GetInOutVehicle(currentVehicleType, isGettingIn: false, force: false, isLeft: true, jump: true, jumpInAir);
            ...
            if (isReplaceOnRagdoll)
            {
                ...
                Transform ragdollHips = GetRagdollHips();
                CameraManager.Instance.SetCameraTarget(ragdollHips);
                Vector3 force = PlayerInteractionsManager.Instance.LastDrivableVehicle.MainRigidbody.linearVelocity * 3f + Vector3.up * 0.2f;
                ragdollHips.GetComponent<Rigidbody>().AddForce(force / 20, ForceMode.VelocityChange);
            }
        ...
        public void GetInOutVehicle(bool isInOut, bool force, DrivableVehicle vehicle, bool crash = false)
        {
            ...
            if (GetComponent<ShootinIKHandler>())
            {
                GetComponent<ShootinIKHandler>().isOnBike = false;
            }
            ...
            {
                animController.MainAnimator.enabled = true;
        ...
        private void EnterToVehicleInstantly(DrivableVehicle vehicle)
        {
            ...
            if (!vehicle.IsControlsPlayerAnimations())
            {
                ...
                PlayerInteractionsManager.Instance.MoveCharacterToSitPosition(PlayerInteractionsManager.Instance.CharacterHips, vehicle.VehiclePoints.DriverPosition, 1f);
                PlayerInteractionsManager.Instance.TweakingSkeleton(PlayerInteractionsManager.Instance.CharacterHips, vehicle.VehiclePoints.DriverPosition, 1f);
            }
        }
        ...
        protected override void OnCollisionSpecific(Collision col)
        {
            if (!IsTransformer && animController.NeedCollisionCheckInCurrentState)
            {
                ...
                {
                    GlobalContants.isWingsOn = false;
                    Debug.Log("GlobalContants.isWingsOn OnDemand");
                    Game.UI.UIGame.Instance.itemParent.SetActive(true);
                    CameraManager.Instance.SetCameraTarget(currentRagdoll.transform.Find("metarig").Find("mixamorig:Hips"));
                    //CameraManager.Instance.SetCameraTarget(currentRagdoll.transform.Find("metarig").Find("habshi@T-Pose").Find("mixamorig:Hips"));
                }
            }
        }
        ...
        protected override void OnDie(DamageType damageType = DamageType.Energy)
        {
            if (!Dead)
            {
                print("Died 1");
                PlayerDieManager.Instance.Links.revivePanel.SetActive(true);
                Time.timeScale = 0;
            }
        }
        ...
        public void Revive()
        {
            ...
            {
                GlobalContants.IsRevived = true;
                RewardedAdsController.Instance.ShowRewarded("Revive");
            }
        }
        ...
        protected override void OnDieSpecific()
        {
            if (!currentRagdoll)
            {
                ReplaceOnRagdoll(canWakeUp: false, IsSwiming);
                return;
            }
        ...
        public override void Resurrect()
        {
            ...
            CharacterSensor.gameObject.SetActive(value: true);
            playerInteractionsManager.GetOutFromVehicle();
            base.enabled = true;
            Dead = false;
            Health.Setup();
            stats.stamina.Setup();
