...
namespace Game.Character.CharacterController
{
    public class TargetManager : MonoBehaviour
    {
        ...
        private void SlowUpdate()
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, Camera.farClipPlane, AimingLayerMask))
            {
        ...
        public CastResult ShootFromCamera(HitEntity owner, Vector3 scatterVector, out Vector3 shotDirVector, float maxShotDistance = 0f, RangedWeapon customWeapon = null, bool humanoidShoot = false)
        {
            ...
            if (!Physics.Raycast(ray, out RaycastHit hitInfo, Camera.farClipPlane, ShootingLayerMask))
            {
            ...
            if (Physics.Raycast(Camera.transform.position, Camera.transform.TransformDirection(Vector3.forward), out hit, Mathf.Infinity , ShootingLayerMask))
            {
        ...
        private CastResult CastWithRay(HitEntity owner, Ray ray, float maxDistance = 0f)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitInfo, maxDistance, ShootingLayerMask))
            {
                ...
                if ((bool)castResult.HitEntity && !castResult.HitEntity.IsDead && FactionsManager.Instance.GetRelations(owner.Faction, castResult.HitEntity.Faction) != 0)
                {
                    ...
                    if (x != null)
                    {
                        ...
                        if (Physics.Raycast(hitInfo.point, ray.direction, out RaycastHit hitInfo2, 4f, mask))
                        {
        ...
        private void LoadAutoAim()
        {
            ...
            if ((bool)AutoAimToggle)
            {
                AutoAimToggle.isOn = UseAutoAim;
            }
        }
