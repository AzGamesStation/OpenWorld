...
namespace Game.Character.CollisionSystem
{
	public class VolumetricCollision : ViewCollision
	{
		...
		public override float Process(Vector3 cameraTarget, Vector3 dir, float distance)
		{
			...
			{
				CollisionClass collisionClass = ViewCollision.GetCollisionClass(hits[l].collider, @string, string2);
				if (hits[l].distance < num2 && collisionClass == CollisionClass.Collision)
				{
					num2 = hits[l].distance;
					value = hits[l].distance - @float + -0.5f;
				}
				if (collisionClass == CollisionClass.IgnoreTransparent)
				{
					UpdateTransparency(hits[l].collider);
				}
			}
