...
namespace Game.Mech
{
	public class MechGunController : MonoBehaviour
	{
		...
		{
			Debug.Log("Shoot From Ranged Weapons");
			TargetManager.Instance.ShootFromCamera(vehStatus.GetVehicleDriver(), rangedWeapon1.ScatterVector, out Vector3 shotDirVector, rangedWeapon1.AttackDistance, rangedWeapon1);
			TargetManager.Instance.ShootFromCamera(vehStatus.GetVehicleDriver(), rangedWeapon2.ScatterVector, out Vector3 shotDirVector2, rangedWeapon2.AttackDistance, rangedWeapon2);
			if (DirectionPermittedForShooting(shotDirVector))
			{
				Debug.Log("Shoot Dir Vector");
				rangedWeapon1.Attack(vehStatus.GetVehicleDriver(), shotDirVector);
			}
			if (DirectionPermittedForShooting(shotDirVector2))
			{
				Debug.Log("Shoot Dir Vector2");
