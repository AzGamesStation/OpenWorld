...
namespace Game.GlobalComponent
{
	...
	public class VehicleCameraMode : CameraMode
	{
		...
		public override Game.Character.Modes.Type Type => Game.Character.Modes.Type.ThirdPersonVehicle;
		...
		private void UpdateDir()
		{
			...
			UpdateAutoReset();
			Game.Character.Utils.Math.ToCartesian(rotX, rotY, out Vector3 dir);
			UnityCamera.transform.forward = dir;
			float float2 = config.GetFloat("Distance");
			float float3 = config.GetFloat("SpeedRollback");
			rollbackDistance = float2 * Mathf.Clamp01(velocityFilter.GetValue().magnitude * float3);
		...
		private void UpdateAutoReset()
		{
			...
			if (resetTimeout <= 0f)
			{
				...
				float rotZ = (0f - config.GetFloat("DefaultYRotation")) * ((float)System.Math.PI / 180f);
				Game.Character.Utils.Math.ToCartesian(num, rotZ, out Vector3 dir);
				Game.Character.Utils.Math.ToCartesian(rotX, rotY, out Vector3 dir2);
		...
		public override void FixedStepUpdate()
		{
			...
			{
				targetPos = Vector3.SmoothDamp(lastTargetPos, Target.position, ref springVelocity, config.GetFloat("Spring"));
		...
		{
			Vector3 a = Target.position - lastTargetPos;
