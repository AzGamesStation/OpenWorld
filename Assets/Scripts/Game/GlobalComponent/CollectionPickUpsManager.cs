	...
	public class CollectionPickUpsManager : MonoBehaviour
	{
		...
		public void ElementWasTaken(GameObject elem)
		{
			...
			if (!(component == null))
			{
				...
				string text = (!string.IsNullOrEmpty(component.m_ShowedName)) ? component.m_ShowedName : collectionType.ToString();
				string specificString = "You have collected " + countAmmount[collectionType] + " / " + totalAmmount[collectionType] + " of " + text;
		...
		{
			IEnumerator enumerator = Enum.GetValues(typeof(CollectionTypes)).GetEnumerator();
			try
			{
				while (enumerator.MoveNext())
				{
					CollectionTypes collectionTypes = (CollectionTypes)enumerator.Current;
					int num = 0;
					int num2 = 0;
					for (int i = 0; i < collectionRespauners.Length; i++)
					{
						...
						if (!(component == null) && component.CollectionType == collectionTypes)
						{
							num++;
							if (collectinBools[i])
							{
					...
					totalAmmount[collectionTypes] = num;
					countAmmount[collectionTypes] = num2;
				}
			}
			finally
			{
		...
		{
			Reward item = new Reward(CollectionTypes.Faberge, totalAmmount[CollectionTypes.Faberge], delegate
			{
				GameEventManager.Instance.Event.PickUpCollectionEvent(CollectionTypes.Faberge.ToString());
			});
			RewardList.Add(item);
			item = new Reward(CollectionTypes.Naxeex, totalAmmount[CollectionTypes.Naxeex], delegate
			{
				if (GameManager.ShowDebugs)
				{
					UnityEngine.Debug.Log("You Have Collected All Naxeex.");
				}
				GameEventManager.Instance.Event.PickUpCollectionEvent(CollectionTypes.Naxeex.ToString());
			});
			RewardList.Add(item);
			item = new Reward(CollectionTypes.RopeHero, totalAmmount[CollectionTypes.RopeHero], delegate
			{
				GameEventManager.Instance.Event.PickUpCollectionEvent(CollectionTypes.RopeHero.ToString());
			});
			RewardList.Add(item);
		}
	}
