	...
	public class MarkForMiniMap : MonoBehaviour
	{
		...
		protected virtual void DrawIcon()
		{
			if (!drawedIcon)
			{
				...
				drawedIconSprite.sortingOrder = 1;
				drawedIcon = gameObject;
				drawing = true;
				if (highLight)
				{
					CircleHighlight circleHighlight = UnityEngine.Object.Instantiate(miniMap.m_HighLightPrefab, gameObject.transform, worldPositionStays: false);
					circleHighlight.transform.localScale = Vector3.one;
					circleHighlight.m_ControlSpriteRender = drawedIconSprite;
				}
			}
		}
		...
		private void DrawArrow(Vector3 position, bool draw)
		{
			...
			if (!drawedArrow)
			{
				...
				drawedArrowSprite.sortingOrder = 1;
				drawedArrow = gameObject;
				if (highLight)
				{
					CircleHighlight circleHighlight = UnityEngine.Object.Instantiate(miniMap.m_HighLightPrefab, gameObject.transform.transform, worldPositionStays: false);
					circleHighlight.transform.localScale = Vector3.one;
					circleHighlight.m_ControlSpriteRender = drawedArrowSprite;
				}
			}
			if (draw)
			{
		...
		private IEnumerator CheckVisibility()
		{
			...
			while (true)
			{
				...
				if ((bool)target)
				{
					...
					vector = new Vector3(x, y, position2.z);
					Vector3 position3 = base.transform.position;
					float x2 = position3.x;
					float y2 = iconY;
					Vector3 position4 = base.transform.position;
					vector2 = new Vector3(x2, y2, position4.z);
					float num = Vector3.SqrMagnitude(vector - vector2);
					if (num > visibleRange * visibleRange && !miniMap.IsFullScreen)
					{
						if (FullMapArrow)
						{
							Vector3 a = (vector2 - vector).normalized * visibleRange;
							DrawArrow(a + vector, draw: true);
						}
						else if (num < NotFullMapVisibleRange * NotFullMapVisibleRange)
						{
							Vector3 a2 = (vector2 - vector).normalized * visibleRange;
							DrawArrow(a2 + vector, draw: true);
						}
						else
						{
