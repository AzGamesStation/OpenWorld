...
namespace Game.Shop
{
	public class SalesManager : MonoBehaviour
	{
		...
		private void UpdateSales()
		{
			if (TimeManager.AnotherDay(lastSalesUpdateTime))
			{
				...
				while (num > 0 && num2 > 0)
				{
					...
					if (!ShopManager.Instance.BoughtAlredy(item) && !item.ShopVariables.isDivision && !item.ShopVariables.HideInShop && item.ShopVariables.price != 0 && !currentSales.ContainsKey(key))
					{
		...
		public int GetSale(int itemID,  Sprite sprite)
		{
			//sprite = new Sprite();
			int num = 0;
			if (currentSales.ContainsKey(itemID))
