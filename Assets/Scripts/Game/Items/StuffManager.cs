...
namespace Game.Items
{
	public class StuffManager : MonoBehaviour
	{
		...
		public static StuffManager Instance => instance ?? (instance = UnityEngine.Object.FindObjectOfType<StuffManager>());
		...
		public void EquipItem(GameItem item, bool equipOnly = false)
		{
			if (item == null)
			{
				CalculateBonusStats(PlayerInteractionsManager.Instance.Player.stats.upgradeValues);
				PlayerInteractionsManager.Instance.Player.UpdateStats(resetCurrent: false);
				return;
			}
			switch (item.Type)
			{
		...
		private void CalculateBonusStats(UpgradeList upgradeValues)
		{
			upgradeValues.ResetBonusValues();
			List<int> equipedSkinIDs = PlayerStoreProfile.CurrentLoadout.GetEquipedSkinIDs();
			int count = equipedSkinIDs.Count;
			for (int i = 0; i < count; i++)
			{
				GameItemSkin gameItemSkin = ItemsManager.Instance.GetItem(equipedSkinIDs[i]) as GameItemSkin;
				if (gameItemSkin != null)
				{
					int num = gameItemSkin.StatAttributes.Length;
					for (int j = 0; j < num; j++)
					{
						StatAttribute statAttribute = gameItemSkin.StatAttributes[j];
						upgradeValues.GetStatMas(statAttribute.StatType).bonusvalue += statAttribute.GetStatValue;
					}
				}
			}
		}
		...
		private void EquipSkin(GameItemSkin skin, bool withoutUpdate = false)
		{
			...
			if (!withoutUpdate)
			{
				...
				UpdateClothes();
				CurrentHelperRagdoll.gameObject.SetActive(true);
				UpdateClothes(CurrentHelperRagdoll);
				CurrentHelperRagdoll.gameObject.SetActive(false);
			}
			PlayerStoreProfile.SaveLoadout();
		}
		...
		{
			StuffHelper targetHelper = customHelper ?? CurrentHelper;
		...
		public void UpdateClothes(StuffHelper customHelper = null, Loadout customLoadout = null, bool isCapActive = false)
		{
			StuffHelper stuffHelper = customHelper ?? CurrentHelper;
						...
						for (int i = 0; i < gameItemClothes.OccupiedSlots.Length; i++)
						{
							...
							if (i == 0)
							{
								if (!(renderer == null))
								{
									...
									if (renderer.gameObject.GetComponentInParent<StuffHelper>())
									{
										for (int j = 0; j < renderer.gameObject.GetComponentInParent<StuffHelper>().SlotRenderers.SkeltonProps.Length; j++)
										{
											if (j == gameItemClothes.index)
											{
												renderer.gameObject.GetComponentInParent<StuffHelper>().SlotRenderers.SkeltonProps[j].SetActive(true);
											}
											else
											{
												renderer.gameObject.GetComponentInParent<StuffHelper>().SlotRenderers.SkeltonProps[j].SetActive(false);
											}
										}
										if (renderer.gameObject.GetComponentInParent<StuffHelper>().SlotRenderers.cape)
										{
											//if (isActiveCap)
											//{
											//	renderer.gameObject.GetComponentInParent<StuffHelper>().SlotRenderers.cape.gameObject.SetActive(true);
											//}
											//else
											//{
											//	renderer.gameObject.GetComponentInParent<StuffHelper>().SlotRenderers.cape.gameObject.SetActive(false);
											//}
											//Debug.Log("Cloth Updating");
											renderer.gameObject.GetComponentInParent<StuffHelper>().SlotRenderers.cape.sharedMaterials = gameItemClothes.SkinMaterials;
										}
									}
								}
							}
		...
		private void ProceedPack(GameItem item)
		{
			...
			{
				ShopManager.Instance.Give(item2);
			}
		}
		...
		private void ClotheAnAccessory(GameItemAccessory accessory, Transform placeholder)
		{
			if (!(accessory == null) && !(placeholder == null))
			{
				...
				{
					GameObject gameObject = UnityEngine.Object.Instantiate(accessory.ModelPrefab, placeholder);
		...
		public void EquipWeapon(GameItemWeapon weaponItem, int slotIndex, bool equipOnly = false)
		{
			...
			if (AlredyEquiped(weaponItem, out string key) && !equipOnly)
			{
		...
		private void ProceedBonus(GameItem item)
		{
			...
			case BonusTypes.Money:
				...
				break;
			case BonusTypes.EXP:
				LevelManager.Instance.AddExperience(gameItemBonus.BonusValue);
				break;
			}
		}
