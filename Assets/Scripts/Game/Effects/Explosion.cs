...
namespace Game.Effects
{
	public class Explosion : Effect
	{
		...
		private void ExplosionProcess(GameObject[] ignoredGameObjects)
		{
			...
			Collider[] array = Physics.OverlapSphere(base.transform.position, ExplosionRange, AffectedLayerMask);
		...
		private void ExplodeHitEntity(Component component)
		{
			...
			hitted.OnHit(DamageType, explosionInitiator, currentDamage, currentTargetPos, currentDirection);
		...
		private void ExplodeHuman(Human human)
		{
			...
			{
				human.ReplaceOnRagdoll(canWakeUp: true, currentDirection * currentForce * ForceMultipliers.human + Vector3.up);
			}
		}
		...
		private void ExplodeNPC(HumanoidStatusNPC npc)
		{
			...
			{
				npc.ReplaceOnRagdoll(canWakeUp: true, currentDirection * currentForce * ForceMultipliers.human + Vector3.up);
			}
		}
