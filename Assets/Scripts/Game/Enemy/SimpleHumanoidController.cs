...
namespace Game.Enemy
{
	public class SimpleHumanoidController : BaseControllerNPC
	{
		...
		private void MoveToPoint(Vector3 point)
		{
            ...
            if (behaiviour == HumanoidBehaiviour.Walk)
            {
				CurrentControlledNpc.NPCAnimator.SetBool("Walk", value: true);
            }
            else
            {
				this.transform.parent.Translate(0, 0, 1 * speed*Time.deltaTime);
		...
		{
			CurrentControlledNpc.ChangeController(BaseNPC.NPCControllerType.Smart, out BaseControllerNPC controller);
