...
namespace Game.Enemy
{
    public class SmartHumanoidController : BaseControllerNPC
    {
        ...
        public override void Init(BaseNPC controlledNPC)
        {
            ...
            {
                GetComponentInParent<HumanoidNPC>().RootModel.GetComponent<CapsuleCollider>().center = new Vector3(0, 0.85f, 0);
                GetComponentInParent<HumanoidNPC>().RootModel.transform.localEulerAngles = new Vector3(0, 0, 0);
            }
        }
        ...
        private void ProceedMove()
        {
            ...
            if (moveToDestination)
            {
                if (NavMeshAgent.hasPath)
                {
                    ...
                    if (!attackEnemy)
                    {
                        vector = CalculateMoveVectorWithObstaclesAvoid(vector, resetEvade: true, 1f);
                    }
                ...
                if (currentActionState == ActionState.RunOut)
                {
                    ...
                    vector = CalculateMoveVectorWithObstaclesAvoid(vector, resetEvade: true, 5f);
                    inputMove.y = moveSpeed;
                    lookPos = rootTransform.position + vector + Vector3.up * 1.4f;
                }
            }
        ...
        private void UpdateEnemyState(float minRangeToTarget)
        {
            ...
            if (!CalculatePathToPoint(enemyTargetPosition) || (flag && sqrMagnitude > attackDistance * attackDistance))
            {
                ...
                Debug.Log("MoveTOdistination0000");
            }
            else
            {
                SetNavMeshPath(path);
        ...
        {
            Physics.Raycast(targetPoint, Vector3.down, out RaycastHit hitInfo, maxDistance, PlayerManager.Instance.DefaulAnimationController.ObstaclesLayerMask);
            return hitInfo;
        }
