...
namespace Game.Enemy
{
	public class RagdollDrowning : MonoBehaviour
	{
		...
		public void Init(Transform hips, float hight)
		{
			...
			this.hips.linearVelocity = Vector3.zero;
		}
		...
		private void FixedUpdate()
		{
			if (isInit)
			{
				...
				Vector3 velocity = hips.linearVelocity;
				float x = Mathf.Clamp(velocity.x, -0.5f, 0.5f);
				Vector3 velocity2 = hips.linearVelocity;
				float y = velocity2.y;
				Vector3 velocity3 = hips.linearVelocity;
				rigidbody.linearVelocity = new Vector3(x, y, Mathf.Clamp(velocity3.z, -0.5f, 0.5f));
