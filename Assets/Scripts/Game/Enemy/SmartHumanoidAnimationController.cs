...
namespace Game.Enemy
{
	public class SmartHumanoidAnimationController : MonoBehaviour
	{
		...
		private class RayHitComparer : IComparer<RaycastHit>
		{
		...
		public AnimationController.LookAtWeights LookAtWeights;
		...
		private bool HasRobotInFront()
		{
			...
			if (Physics.Raycast(ray, out RaycastHit hitInfo, 5f, TargetManager.Instance.ShootingLayerMask) && (hitInfo.collider.CompareTag("Player") || hitInfo.collider.CompareTag("Robot")))
			{
				return true;
			}
			return false;
